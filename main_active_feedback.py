...
def retrieval_pipeline(args, index, ncomb, hyperparams):
    ...
    ### ---------------- Filter selection  ---------------------
    if args.filter_function == "ActiveFeedback":
        filtering = dimension_filters.ActiveFeedback(qrels=qrels, docs_encoder=docs_encoder, qrys_encoder=qrys_encoder,
                                                      run=qrels)

    elif args.filter_function == "NegActiveFeedback":
        filtering = dimension_filters.NegActiveFeedback(qrels=qrels, docs_encoder=docs_encoder, qrys_encoder=qrys_encoder,
                                                         run=qrels, hyperparams=hyperparams)
    ...
    for measure_name in ['nDCG@10', 'AP']:
        ...
        save_filename = f"{args.datadir}/performance/active_feedback/{measure_folder}/{args.collection}_{args.model_name}_{args.filter_function}_{measure_name}_{ncomb}.csv"
